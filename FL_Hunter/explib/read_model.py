...
from FL_Hunter.models import FL_HunterModuleProtocol, FL_HunterModelInterface
...
def read_model(
        ...
        override_config: Dict=None  # 字典类型，表示要覆盖的模型配置参数
) -> Tuple[FL_HunterModelInterface, np.ndarray, np.ndarray]:
    #############################################################################################################
    ...
    config = FL_HunterConfig().load(path=f"{exp_dir}/config")
    config.augmentation = False  # it modifies the train dataset

    for k, v in override_config.items():
        setattr(config, k, v)

    model = FL_HunterModelInterface.load_from_checkpoint(
    ...
    from FL_Hunter.models.interface.callbacks import CFLLoggerCallback
