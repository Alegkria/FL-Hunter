...
from FL_Hunter.dataset import FL_HunterDataset, SKLEARN_DATASET_TYPE
from FL_Hunter.explanability.fe_decoder import FeatureExtractorDecoder
from FL_Hunter.models import FL_HunterModelInterface
...
class Encoder(th.nn.Module):
    ...
    def forward(self, x: List[th.Tensor]):
        ...
        feat = th.cat(feat_list, dim=-2)
        return feat


class FeatureExtractorDecoderModelInterface(pl.LightningModule): #继承
    def __init__(self, model: FL_HunterModelInterface, *args: Any, **kwargs: Any):
...
def train_feature_extractor_decoder(
        model: FL_HunterModelInterface
) -> FeatureExtractorDecoder:
    dataloader = model.train_dataloader()
    assert isinstance(dataloader.dataset, FL_HunterDataset)
...
def select_useful_columns(
    ...
    if mode == 'full':
        ...
        from FL_Hunter.explanability import SimpleFCParameters
        default_fc_parameters = SimpleFCParameters()
    elif mode == 'simple_fctype':
        from FL_Hunter.explanability.ts_feature import SimpleFCTypeFCParameters
...
def dt_follow(
    ...
    for node_type in cdp.failure_classes:
        ...
        if np.max(train_y) < len(class_names) - 1:
            continue
