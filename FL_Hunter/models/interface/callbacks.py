...
from FL_Hunter.evaluation_metrics import top_1_accuracy, top_2_accuracy, top_3_accuracy, top_k_accuracy, MAR, get_rank
from FL_Hunter.models.interface.model_interface import FL_HunterModelInterface
...
class CFLLoggerCallback(pl.Callback):
    ...
    def on_train_epoch_start(self, trainer: 'pl.Trainer', pl_module: 'pl.LightningModule') -> None:
        ...
        # 在每个训练 epoch 开始时调用。根据设定的频率判断是否需要在该 epoch 显示指标信息

    def on_train_epoch_end(
            self, trainer: 'pl.Trainer', pl_module: FL_HunterModelInterface, unused: Optional = None
    ) -> None:
    ...
    # 在每个训练 epoch 结束时调用。显示该 epoch 的训练指标信息，如损失、准确率等。

    def on_validation_epoch_end(self, trainer: 'pl.Trainer', pl_module: FL_HunterModelInterface) -> None:
        self.print(
            f"epoch={pl_module.current_epoch:<5.0f} "
    ...
    # 在每个验证 epoch 结束时调用。显示该 epoch 的验证指标信息，如损失、准确率等
'''
...
'''
    ...
    def on_test_epoch_end(self, trainer: pl.Trainer, pl_module: FL_HunterModelInterface) -> None:
        ...
        # 使用了Python的reduce函数和列表推导式，目的是获取训练数据中所有的根因实例的ID
        ...
        self.print(f"\n{output.getvalue()}")
    # 在每个测试 epoch 结束时调用。显示测试集的指标信息，如准确率、排名等
# 该代码定义了一个自定义的 PyTorch Lightning 回调函数 CFLLoggerCallback，
# 用于在训练和验证过程中打印训练和验证的指标信息

class TestCallback(pl.Callback):
    def __init__(self, model: FL_HunterModelInterface, second_freq: float = 30, epoch_freq: int = 100):
