...
from FL_Hunter.models import FL_HunterModuleProtocol
from failure_instance_feature_extractor import FIFeatureExtractor
from FL_Hunter.models.node_weight_predictor import NodeWeightPredictor
from FL_Hunter.models.attention_weight_predictor import AttentionWeightPredictor
from utils import SequentialModelBuilder


class GAT(FL_HunterModuleProtocol):
    @profile
    def __init__(
            self,
            node_types: List[str],
        ...
        #self.predictor = AttentionWeightPredictor(feature_size=1632, n_head=8)
    ...
    def forward(self, x: List[th.Tensor], graphs: List[dgl.DGLGraph]):
        ...
        node_weights = self.predictor(agg_feat, agg_feat, agg_feat)
