...
class FL_HunterDataset(th.utils.data.Dataset):
    @profile
    def __init__(
    ...
    def to(self, device: Union[th.device, str]) -> 'FL_HunterDataset':
        self._device = device
...
def prepare_sklearn_dataset(
        cdp: FDG, config: FL_HunterConfig, cache: Cache, mode: str
) -> SKLEARN_DATASET_TYPE:
    from FL_Hunter.explanability import prepare_ts_feature_dataset
