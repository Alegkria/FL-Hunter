...
from FL_Hunter.models import FL_HunterModelInterface, get_GAT_model
from FL_Hunter.models.interface import CFLLoggerCallback
...
def count_epoch_time(n_instances: int, n_metrics: int, n_failures: int) -> float:
    config = FL_HunterConfig().parse_args(
        shlex.split(
    ...
    model = FL_HunterModelInterface(config, get_GAT_model)
